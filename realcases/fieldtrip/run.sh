...
for i in $(seq 1 $njobs); do
    hid=$( echo "$i % ${#hlist[@]}" | bc )
    [ $i -le 195 ] &&
        echo "${PWD}/job.sh $i" | qsub -q tgtest -N bench_mous_${ses}.${i} -l "walltime=60:00,mem=4gb,nodes=${hlist[$hid]}" ||
        echo "${PWD}/job.sh $i" | qsub -q tgtest -N bench_mous_${ses}.${i} -l "walltime=60:00,mem=4gb,nodes=1:ppn=1"
done
