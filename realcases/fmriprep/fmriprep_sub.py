...
def main(bidsdir: str, outputdir: str, workdir_: str, subject_label=(), force=False, mem_mb=18000, file_gb_=50, argstr='', dryrun=False, skip=True):
    ...
    for n, sub_dir in enumerate(sub_dirs,1):
        ...
        if force or not report.is_file() or not all(sessions):
            ...
            command = """qsub -q tgtest -l walltime=70:00:00,mem={mem_mb}mb,{file_gb}epilogue={epilogue} -N fmriprep_sub-{sub_id} <<EOF
                         cd {pwd}
                         ...
                         .format(pwd         = Path.cwd(),
                                 fmriprep    = f'unset PYTHONPATH; export PYTHONNOUSERSITE=1; singularity run --bind /cephfs:/cephfs --cleanenv {os.getenv("DCCN_OPT_DIR")}/fmriprep/{os.getenv("FMRIPREP_VERSION")}/fmriprep-{os.getenv("FMRIPREP_VERSION")}.simg',
                                 bidsdir     = bidsdir,
                                 outputdir   = outputdir,
                                 workdir     = workdir,
                                 sub_id      = sub_id[4:],
